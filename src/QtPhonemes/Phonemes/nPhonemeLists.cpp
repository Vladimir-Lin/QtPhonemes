...
extern const unsigned char PausePhonemes[8] ;

N::PhonemeLists:: PhonemeLists         (void)
                : QList<PhonemeList *> (    )
{
}

N::PhonemeLists:: PhonemeLists         (int total)
                : QList<PhonemeList *> (         )
{
  PhonemeList * pl           ;
...
int N::PhonemeLists    :: Substitute (
    PhonemeLists        & list       ,
    Phonemes            & phonemes   ,
    PhonemeReplacements & replace    )
{ // Copy the phonemes list and perform any substitutions that are required for the current voice
  int       ix                                                                ;
  int       k                                                                 ;
  int       replace_flags                                                     ;
  int       word_end                                                          ;
  ...
  for ( ix = 0 ; ix < count() ; ix++ )                                        {
    if ( ! ( ME[ix] -> synthflags & SFLAG_SWITCHED_LANG))                     {
      // don't do any substitution if the language has been temporarily changed
      if ( ix < ( count() -1 ) ) next = phonemes [ ME [ ix + 1 ] -> phcode ]  ;
      word_end = 0                                                            ;
      // this phoneme is the end of a word
      if ( ME[ix+1] -> sourceix > 0                          ) word_end = 1 ; else
      if ( ( ( NotNull(next) ) && ( next -> isPause() ) ) ) word_end = 1      ;
      // check whether a Voice has specified that we should replace this phoneme
      for ( k = 0 ; k < replace.count() ; k++ )                               {
        if ( ME[ix] -> phcode == replace[k]->Old)                              {
          replace_flags = replace[k]->Type                                     ;
          if ( ( replace_flags & 1 ) && ( word_end == 0 ) ) continue          ;
          // this replacement only occurs at the end of a word
          if ( ( replace_flags & 2             )                             &&
             ( ( ME[ix] -> stresslevel & 0x7 ) > 3) ) continue                ;
          // this replacement doesn't occur in stressed syllables
          if ( ( replace_flags & 4 ) && ( ME[ix] -> sourceix == 0 ) ) continue   ;
          // this replacement only occurs at the start of a word
          // substitute the replacement phoneme
          ME[ix] -> phcode = replace[k] -> New                                      ;
          if ( ( ME[ix] -> stresslevel > 1 )                                    &&
               phonemes[ME[ix] -> phcode] -> isUnstressed()) ME[ix] -> stresslevel = 0 ;
          // the replacement must be unstressed
          break                                                               ;
        }                                                                     ;
      }                                                                       ;
      if (ME[ix] -> phcode == 0) continue                                     ;
      // phoneme has been replaced by NULL, so don't copy it
    }                                                                         ;
    // copy phoneme into the output list
    PhonemeList * pl                                                          ;
    pl         = new PhonemeList ( )                                          ;
    *pl        = *(ME[ix])                                                    ;
    pl -> ph   = phonemes [ pl -> phcode ]                                    ;
    pl -> type = pl->ph->Type                                                 ;
    list << pl                                                                ;
  }                                                                           ;
  return list.count() ;
       ...
       int                start_sentence )
{
  ...
  ///////////////////////////////////////////////////////////////
  ME[ix] -> newword    = 2                                      ; // end of clause
  ME[ix] -> phcode     = Speak::PcPause                         ;
  ME[ix] -> type       = Speak::PAUSE                           ; // terminate with 2 Pause phonemes
  ME[ix] -> length     = post_pause                             ; // length of the pause, depends on the punctuation
  ME[ix] -> sourceix   = end_sourceix                           ;
  ME[ix] -> synthflags = 0                                      ;
  ME[ix] -> ph         = phonemes [ Speak::PcPause ]            ;
  ///////////////////////////////////////////////////////////////
  PhonemeList * npl                                             ;
  npl               = new PhonemeList ( )                       ;
  npl -> phcode     = Speak::PcPause                            ;
  npl -> type       = Speak::PAUSE                              ;
  npl -> length     = 0                                         ;
  npl -> sourceix   = 0                                         ;
  npl -> synthflags = 0                                         ;
  npl -> ph         = phonemes [ Speak::PcPauseShort ]          ;
  ///////////////////////////////////////////////////////////////
  push_back ( npl )                                             ;
}
...
int N::PhonemeLists :: VowelPosition(int index)
{
  int count = 0                                          ;
  while ( index >= 0 )                                   {
    if (ME[index] -> ph -> Type == Speak::VOWEL) count++ ;
    if (ME[index] -> sourceix != 0) break                ;
    index--                                              ;
  }                                                      ;
  return count                                           ;
}
