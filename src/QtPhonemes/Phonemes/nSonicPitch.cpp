...
N::SonicPitch:: SonicPitch           (int SampleRate,int NumChannels)
              : inputBuffer          (NULL                          )
              , outputBuffer         (NULL                          )
              , pitchBuffer          (NULL                          )
              , downSampleBuffer     (NULL                          )
              ...
              , prevMinDiff          (0                             )
{
  Set ( SampleRate , NumChannels ) ;
}

N::SonicPitch::~SonicPitch (void)
{
  #define CLEAR(ITEM) if (NotNull(ITEM)) delete ITEM ; ITEM = NULL
    CLEAR ( inputBuffer      ) ;
    CLEAR ( outputBuffer     ) ;
    CLEAR ( pitchBuffer      ) ;
    CLEAR ( downSampleBuffer ) ;
  #undef  CLEAR
}

bool N::SonicPitch::Set(int SampleRate,int NumChannels)
{
  int bs           = NumChannels                  ;
  minPeriod        = SampleRate / SONIC_MAX_PITCH ;
  maxPeriod        = SampleRate / SONIC_MIN_PITCH ;
  maxRequired      = maxPeriod  * 2               ;
  bs              *= maxRequired                  ;
  inputBufferSize  = maxRequired                  ;
  inputBuffer      = new short    [ bs          ] ;
  outputBufferSize = maxRequired                  ;
  outputBuffer     = new short    [ bs          ] ;
  pitchBufferSize  = maxRequired                  ;
  pitchBuffer      = new short    [ bs          ] ;
...
void N::SonicPitch::Scale(short * samples,int numSamples,float volume)
{
